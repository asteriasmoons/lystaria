...
function readFrontmatter(filePath) {
  ...
  const excerpt = pickFirst(fm, ["description", "excerpt", "summary"]);
  const image = pickFirst(fm, [
    "image",
    "cover",
    "banner",
    "thumbnail",
...
(async () => {
  ...
  for (const file of files) {
    ...
    else if (isWithin(BLOG_CONTENT_DIR_2, file))
      contentDir = BLOG_CONTENT_DIR_2;
    else continue;
    ...
    const { title, excerpt, image } = readFrontmatter(absPath);
